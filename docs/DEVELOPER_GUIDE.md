# Developer Guide: Sovereign Cloud Architect
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/cloud-architect.git
cd cloud-architect
...
# 5. Verify
curl http://localhost:8784/health
```

## Environment Configuration

```bash
export SBB_CLOUD_PROVIDER=your_value_here
export SBB_TERRAFORM_BACKEND=your_value_here
export SBB_COST_ALERT_THRESHOLD=your_value_here
export SBB_COMPLIANCE_LEVEL=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-cloud-architect)
...
```bash
# Build
docker build -t sovereign-cloud-architect:dev .

# Run with env file
docker run --env-file .env -p 8784:8784 sovereign-cloud-architect:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-cloud-architect
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8784`

### Triggering Directly

```bash
curl -X POST http://localhost:8784/api/v1/execute \
...
requests.post(N8N_EVENT_BUS_URL, json={
    "source": "sovereign-cloud-architect",
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8784` in use | `lsof -i :8784` | Kill PID or change `SERVICE_PORT` |
